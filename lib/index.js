...
async function assertOwnerOnly(filename) {
	...
	if ((mode & 63) !== 0) throw new AccountError("STORAGE", `dsh-account: ${filename} is readable beyond its owner (mode ${(mode & 511).toString(8)}); run "chmod 600 ${filename}" before starting again`);
}
...
function parseDocument(text, filename) {
	...
	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new AccountError("STORAGE", `dsh-account: ${filename} must contain an object`);
	const document = value;
	if (document["version"] !== FORMAT_VERSION) throw new AccountError("STORAGE", `dsh-account: ${filename} has unsupported accounts format version ${String(document["version"])}`);
	if (Object.keys(document).some((key) => key !== "version" && key !== "providers")) throw new AccountError("STORAGE", `dsh-account: ${filename} contains an unknown top-level field`);
	const providers = document["providers"];
	if (typeof providers !== "object" || providers === null || Array.isArray(providers)) throw new AccountError("STORAGE", `dsh-account: ${filename} providers must be an object`);
	for (const [providerId, raw] of Object.entries(providers)) {
		if (typeof raw !== "object" || raw === null || Array.isArray(raw)) throw new AccountError("STORAGE", `dsh-account: ${filename} provider "${providerId}" must be an object`);
		const entry = raw;
		if (Object.keys(entry).some((key) => key !== "accounts" && key !== "active")) throw new AccountError("STORAGE", `dsh-account: ${filename} provider "${providerId}" has an unknown field`);
		const accounts = entry["accounts"];
		if (typeof accounts !== "object" || accounts === null || Array.isArray(accounts)) throw new AccountError("STORAGE", `dsh-account: ${filename} provider "${providerId}" accounts must be an object`);
		for (const [accountId, credential] of Object.entries(accounts)) if (!isCredential(credential)) throw new AccountError("STORAGE", `dsh-account: ${filename} provider "${providerId}" account "${accountId}" has an invalid credential`);
		const active = entry["active"];
		if (active !== void 0 && (typeof active !== "string" || !(active in accounts))) throw new AccountError("STORAGE", `dsh-account: ${filename} provider "${providerId}" active account "${String(active)}" is not among its accounts`);
	}
	return document;
}
...
function buildOAuthAdapter(provider, name, activeStore) {
	...
	const profiles = /* @__PURE__ */ new Map([[provider.id, {
		...
		retryPolicy: resolveRetryPolicy(void 0, `dsh-account ${provider.id} retryPolicy`),
...
function apply(ctx) {
	...
	if (registered > 0) ctx.logger.info("dsh-account: registered %d OAuth provider adapter(s)", registered);
