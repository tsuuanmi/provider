...
async function switchActiveAccount(ctx, store, providerId, accountId) {
	...
	if (providerId === CODEX_PROVIDER_ID && credential?.type === "oauth") await ctx.credentials.set(CODEX_SUBSCRIPTION_OAUTH_REF, JSON.stringify(credential));
}
/**
...
async function clearActiveAccountCredential(ctx, providerId) {
	...
	if (providerId === CODEX_PROVIDER_ID) await ctx.credentials.unset(CODEX_SUBSCRIPTION_OAUTH_REF);
}
//#endregion
