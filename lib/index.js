...
function vaultAccountId(providerAccountId) {
...
function isLegacyVaultLabel(label) {
...
*/
function matchVaultAccount(accounts, credential, providerAccountId) {
	...
	if (accountId === void 0) return void 0;
	const legacyMatches = accounts.filter((account) => account.credential.accountId === accountId && isLegacyVaultLabel(account.label));
	return legacyMatches.length === 1 ? legacyMatches[0] : void 0;
}
...
async function syncCodexSubscriptionAccount(ctx, credential, label) {
	...
	await records.modifyRecord(CODEX_VAULT_KEY, async (current) => {
		...
		if (current === void 0) return grantRecord({
		...
		if (match?.id === id && record.payload.activeId === id) return void 0;
		const accounts = match === void 0 ? [...record.payload.accounts, {
			id,
			label: normalizedLabel,
			credential
		}] : record.payload.accounts.map((account) => account.id === match.id ? {
			...account,
			id,
			label: normalizedLabel
		} : account);
...
*/
async function removeCodexSubscriptionAccount(ctx, credential, providerAccountId) {
	if (credential.type !== "oauth" || !isOAuthCredentialShape(credential)) return;
	const records = recordsApi(ctx);
	if (records === void 0) return;
	let skipped;
